...
public class Atomic {
	...
	/**
	 * AtomicMarkableReference 带有一个bool附属标记的对象CAS 一个bool标记可以防止ABA吗？
	 */
	static void testAtomicMarkableReference() {
		...
		boolean b = markRef.compareAndSet(old/* 旧对象 */, newObj, oldFlag/* 旧标记 */, !oldFlag/* 更新标记 */); // 如果在此操作之前，markRef
																										// CAS
																										// 了两次，岂不是没有防止ABA问题？
	...
	/**
	 * AtomicStampedReference 用于对象CAS，带有int标记位的。只要int不溢出，理论上保证不会出现ABA问题
	 */
